...
""";
    ...
    public static void main(String[] args) {
      ...
      //  boolean b = isCorrectlyOrdered(left, right);
       // System.out.println(b);
    }

    private static boolean isCorrectlyOrdered(Element left, Element right) {

        if (left instanceof ListElement && !(right instanceof ListElement)) {
            right = new ListElement(((NumberElement) right).value);
        }
        if (right instanceof ListElement && !(left instanceof ListElement)) {
            left = new ListElement(((NumberElement) left).value);
        }

        var leftList = (ListElement)left;
        var rightList = (ListElement)right;

        if (rightList.elements.size() == 0 && leftList.elements.size() != 0) {
            return false;
        }


        for (int i = 0; i < leftList.elements.size(); i++) {
            var leftElement = leftList.elements.get(i);
            if(rightList.elements.size() == i) {
                return false;
            }
            var rightElement = rightList.elements.get(i);


            //Both numbers
            if (leftElement instanceof NumberElement leftNumber && rightElement instanceof NumberElement rightNumber) {
                if (leftNumber.value > rightNumber.value) {
                    return false;
                }
            }
            if (leftElement instanceof ListElement leftListElement && rightElement instanceof ListElement rightListElement) {
                if(!isCorrectlyOrdered(leftListElement, rightListElement)) {
                    return false;
                }
            }

            if (leftElement instanceof ListElement leftListElement && !(rightElement instanceof ListElement )) {
                if (!isCorrectlyOrdered(leftListElement, new ListElement(((NumberElement)rightElement).value))) {
                    return false;
                }
            }
            if (rightElement instanceof ListElement leftListElement && !(leftElement instanceof ListElement )) {
                if (!isCorrectlyOrdered(new ListElement(((NumberElement)leftElement).value),rightElement)) {
                    return false;
                }
            }
        }
